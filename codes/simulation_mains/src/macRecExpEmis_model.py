...
class MACRecourseModelExpEmis():
    """Marginal abatement cost curve model class with an expectation objective function and 
    a growing emissions baseline.
    ...
    def __init__(self, cal, N_samples, method):
        ...
        self.method = method # method to evaluate expectation operator
    ...
    def _save_output(self):
        ...
        cwd = os.getcwd()
        path = ''.join([cwd, '/data/output/', self.cal, '_N', str(self.N_samples), '_method', str(self.method), '_macrecexpemis_output.nc'])
