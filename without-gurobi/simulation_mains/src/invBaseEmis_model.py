...
from .macBaseEmis_model import MACBaseModelEmis

class INVBaseModelEmis():
    """Abatement investment model class with growing emissions baselines.
    ...
    def _import_calibration(self):
        ...
        # emissions growth rate (per year fractional increase)
        self.g = df_glob['g'].values[0]
        ...
        # include growth in emissions
        self.abar_sec = df_secs.loc['abar'].values
    ...
    def _cal_model(self, save_cal):
        ...
        sol = root(self._get_diff, x0=40000.0, args=(mac_total_cost))
        ...
        if save_cal:
            df = pd.DataFrame([self.gbars, self.abars.value[:,0], self.deltas.value, self.a_0s.value, self.powers, self.cbars.value],
    ...
    def _get_diff(self, ci, mac_total_cost):
        ...
        diff = mac_total_cost - self.prob.value
    ...
    def solve_opt(self, save_output=True, verbose=True, cal_purposes=False):
        ...
        times = np.arange(0, self.T.value+1, self.dt.value)
        growth = (1 + self.g)**times
        growth[growth > (1+self.g)**30] = (1+self.g)**30 # no growth after 2050
        abar = self.abar_sec[:, None] * growth # get growth in each sector at rate g

        self.abars = cp.Parameter((self.N_secs, len(times)), nonneg=True,
                                  value=abar)
        ...
        # else, print the outcome of the optimization
            ...
            print("Time zero SCC: ", -1*self.constraints[-1].dual_value[0])
    ...
    def _save_output(self):
        ...
        ds = xr.Dataset(data_vars={'investment': (['sector', 'time'], self.x.value),
                        ...
                        'abars': (['sector', 'time_state'], self.abars.value),
    ...
    def _get_flux_rhs(self):
        ...
        flux = cp.sum(self.abars[:, :-1] - self.a[:, :-1], axis=0)
        return flux
