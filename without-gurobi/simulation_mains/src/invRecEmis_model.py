"""Abatement Investment model class with recourse and increasing emissions baselines.
...
from .invBaseEmis_model import INVBaseModelEmis

class INVRecourseModelEmis():
    ...
    def _import_calibration(self):
        ...
        try:
            self.cbars = cp.Parameter(self.N_secs, nonneg=True, 
                                      value=df_secs.loc['cbar'].values/1e5)
    ...
    def _process_opt_output(self, print_outcome=False):
        ...
        for per in range(self.N_periods):
            ...
            tmp_ds = xr.Dataset(data_vars={'investment': (['state', 'sector', 'time'], self.x_proc[per]),
                        ...
                        'cbars': (['sector'], self.cbars.value * 1e5),
    ...
    def _save_output(self):
        ...
        cwd = os.getcwd()
        path = ''.join([cwd, '/data/output/', self.cal, '_', self.rec_cal, '_', self.method, '_invrecemis_output.nc'])
