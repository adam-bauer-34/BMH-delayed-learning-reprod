...
class MACBaseModelEmis():
    """Marginal abatement cost curve model class with growing emissions baselines.
    ...
    def _import_calibration(self):
        ...
        abar_sec = df_secs.loc['abar'].values
        times = np.arange(0, self.T.value, self.dt.value)
        growth = (1 + self.g)**times
        growth[growth > (1+self.g)**30] = (1+self.g)**30 # no growth after 2050
        abar = abar_sec[:, None] * growth # get growth in each sector at rate g

        self.abars = cp.Parameter((self.N_secs, len(times)), nonneg=True, 
                                  value=abar) # emissions rates by sector, time dim has to be same as abatement later on
    ...
    def _save_output(self):
        ...
        ds = xr.Dataset(data_vars={'abatement': (['sector', 'time'], self.a.value),
                        ...
                        'abars': (['sector', 'time'], self.abars.value)},
    ...
    def _get_flux_rhs(self):
        ...
        # emissions flux
        flux = cp.sum(self.abars - self.a, axis=0)
        return flux